{{--
    Student exam result review — questions, chosen answers and verdicts.

    Rendered by App\Http\Controllers\Student\StudentExamController::result()
    through the `student.exams.result` route (linked from the dashboard's
    "آخرین نتایج" list). Read-only mirror of the consultant's
    consultant/students/exam-result.blade.php: same hero + quiz-card markup and
    shared styles from resources/css/app.css, with a back link to the student
    dashboard instead of the consultant's student workspace.

    The completion date is Jalali-rendered by
    resources/js/features/student-dashboard.js (fa-date rewrite).
--}}
...
@endphp
<p class="result-back">
    <a href="{{ route('student.dashboard') }}"><i class="fas fa-arrow-right"></i> بازگشت به داشبورد</a>
</p>
            ...
            <ul>
                ...
                @if($attempt->completed_at)
                    <li>
                        ...
                        <time class="fa-date" datetime="{{ $attempt->completed_at->format('Y-m-d\TH:i') }}Z">{{ persian_digits($attempt->completed_at->format('Y/m/d')) }}</time>
                    </li>
                @endif
            </ul>
        </div>
