{{-- resources/views/public/sections/ecosystem/default.blade.php — copy + orbit --}}
@php
    $eco       = $cfg;
...
@endphp
<section id="{{ $eco['id'] ?? 'ecosystem' }}" class="lp-section lp-section--ecosystem">
    <div class="landing-container grid md:grid-cols-2 gap-16 items-center">
        ...
        <div class="flex flex-col gap-6 {{ $reveal ? 'reveal fade-left' : '' }}" style="{{ $textFirst ? 'order:1' : '' }}">
            <h2 style="font-size:var(--h2-size);font-weight:var(--heading-weight);letter-spacing:var(--heading-letter-spacing);text-transform:var(--heading-transform)">{{ $eco['heading'] }}</h2>
