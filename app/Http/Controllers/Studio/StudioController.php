...
/**
 * The Theme Studio: a schema-driven editor over the tenant's runtime config
 * layer (website_configs.layout_config) and the per-user personal layer
 * (preferences.site).
 *
 * Standalone page: the consultant dashboard opens it at /studio in a new tab.
 * It renders through the platform-only `layouts.studio` shell, which includes
 * no tenant theme partials — editing a tenant theme can never restyle the
 * editor itself. Same host on purpose: IdentifyTenant resolves the tenant from
 * the Host header, so tenant resolution, session cookies, uploads and the
 * same-origin preview iframe all keep working unchanged.
 *
...
class StudioController extends Controller
{
    public function index(Request $request): View
    {
        ...
        return view('studio.index', [
            ...
            'previewing' => $request->session()->has(ApplyPersonalTheme::previewSessionKey()),
        ]);
    }
    ...
    public function live(StudioSaveRequest $request): JsonResponse
    {
        ...
        $previewKey = ApplyPersonalTheme::previewSessionKey();

        if ($diff === []) {
            $request->session()->forget($previewKey);
        } else {
            $request->session()->put($previewKey, self::nestedLayer($diff));
        }
    ...
    {
        session([ApplyPersonalTheme::previewSessionKey() => self::nestedLayer($diff)]);
